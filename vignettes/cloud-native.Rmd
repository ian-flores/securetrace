...
`otlp_format_trace()` is the pure function underneath the exporter. It
converts a trace list into the OTLP `ExportTraceServiceRequest` JSON
structure without sending anything:
...
the buffer reaches `batch_size`. This reduces HTTP overhead for high-throughput
agents. Use `flush_otlp()` to force-send any buffered traces -- for example,
at process exit to ensure nothing is lost:
...
Prometheus is a pull-based monitoring system. securetrace can expose agent
metrics (span counts, token usage, cost, duration histograms) in Prometheus
...
must propagate across process boundaries. securetrace implements the
