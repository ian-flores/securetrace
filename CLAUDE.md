...
An R package for observability and tracing of LLM agent workflows.
Structured traces with spans, token/cost accounting, latency monitoring,
and JSONL export.
...
- `R/exporter.R` – jsonl_exporter(), console_exporter(),
  multi_exporter()
- `R/context.R` – with_trace(), with_span(), current_trace(),
  current_span()
- `R/integration.R` – trace_llm_call(), trace_tool_call(),
  trace_guardrail(), trace_execution()
